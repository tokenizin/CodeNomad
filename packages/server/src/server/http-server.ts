...
export function createHttpServer(deps: HttpServerDeps) {
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    const publicApiPaths = new Set(["/api/auth/login", "/api/auth/quick-login", "/api/auth/token", "/api/auth/status", "/api/auth/logout", "/api/tokidapp/status", "/api/client-connections/pong"])

...
async function startVoiceRealtimeSession(
  ...
  if (!getRealtimeSession(sessionId)) {
    ...
    createRealtimeSession(
      ...
      // onUserTranscriptDelta — stream partial transcription to client for real-time chat display
      (delta) =>
        socketRef.send(JSON.stringify({ type: "user_transcript_partial", delta })),
...
async function routeMessage(
  ...
  if (hasWord("investigate") || hasWord("search") || hasWord("find") || hasWord("look")) {
    ...
    const url = urlMatch ? urlMatch[0] : "https://starpages.org"
...
export function registerFileUploadRoutes(app: FastifyInstance) {
  ...
  // GET /api/tokidapp/files/generated/:filename
  // Serves files generated by the concierge (Mermaid diagrams, documents, etc.)
  app.get("/api/tokidapp/files/generated/:filename", async (request, reply) => {
    try {
      const { filename } = request.params as { filename: string }
      // Path matches codebase-tools.ts generateFile
      const codenomadRoot = path.resolve(import.meta.dirname, "..", "..", "..", "..", "..", "..")
      const generatedDir = path.resolve(codenomadRoot, "public", "generated")
      const normalized = path.normalize(filename).replace(/^(\.\.(\/|\\|$))+/, "")
      const filePath = path.join(generatedDir, normalized)
      ...
      const ext = path.extname(filename).toLowerCase()
      const mimeMap: Record<string, string> = {
        ".mmd": "text/plain", ".svg": "image/svg+xml", ".txt": "text/plain",
      ...
      reply.code(500)
      return { error: "Failed to serve generated file" }
    }
  })
}
